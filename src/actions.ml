...
(** The phase-dependent action pane (propose a team / vote on a proposal / vote on a mission
    / assassinate). Only the active phase's component is instantiated via [match%sub]; the
    individual panes are private and dispatched through {!action_pane}. *)
...
let team_proposal_action ~selected (local_ graph) =
  ...
  | Some g ->
    ...
    let body =
      ...
      else {%html.jsx|<div *{[ Ui.center ]}>%{textf "Waiting for %s to propose a team of %d" (Option.value g.current_proposer ~default:"") team_size}</div>|}
    in
...
let mission_action (local_ graph) =
  ...
  | Some g ->
    ...
    let body =
      ...
      then
        N.div
          [ (if String.is_empty error then N.none else {%html.jsx|<div *{[ Ui.field_error; Ui.center ]}>#{error}</div>|})
          ; div ~attrs:[ Ui.row; Ui.between ]
