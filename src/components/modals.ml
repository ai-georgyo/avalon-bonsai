...
(* The event modals are driven by the model's [modal] field rather than a trigger button, so
   we map it to an option and let {!Ui.modal} portal the matching dialog into the top layer. *)
