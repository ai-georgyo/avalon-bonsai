  ...
  | Some g ->
    ...
    let tab idx (mission : mission) =
      let base =
        ...
        | M_pending ->
          [ fa ~color:(if is_future idx then "gray" else "black") "far" "fa-circle"; spanc ~attrs:[ Ui.layers_text ] [ N.text (Int.to_string mission.team_size) ] ]
      ...
      btn ~attrs:(Style.tab_mission :: (if active = idx then [ Ui.tab; Ui.tab_active ] else [ Ui.tab ])) ~on_click:(set_active idx) [ icon ]
    in
    let panel idx (mission : mission) =
      ...
      in
      let log = if (Game.data g).in_game_log then Summary_table.mission_summary_table ~players:(Game.players g) ~missions:[ mission ] ~roles:None ~mission_votes:None else N.none in
      card ~attrs:[ bg; Style.mission_panel ] [ card_text ~attrs:[ Ui.caption ] [ header; detail; log ] ]
    in
